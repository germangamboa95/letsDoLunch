...
var results;
var urlQuery =
  "https://letshavelunchserver.herokuapp.com/api/" + sessionId + "/get_res";
let total_votes;
let winner;
...
function resultsOne() {
  ...
  let key = getKeyByValue(
    results.votes,
    Math.max(...Object.values(results.votes))
  );
...
function createRes(data, numVoters) {
  ...
  data.forEach((item, index) => {
    ...
    <div><p><span id="rest-${index}">Restraunt Name:</span><span class='right' id="vots"> ${
      restaurantVotes[index]
    }/ ${numVoters}</p></div>
...
function topRes(data) {
  ...
  <div class="card medium mb-2">
    <div class="card-image">
      ...
      <span class="card-title">Top choice: <span id="locName">${
        data.name
      }</span></span>
      </div>
      <div class="card-content">
        ...
        <div class="card-action">
          <a class="waves-effect waves-light btn right btn-large green accent-4" href="https://www.google.com/maps/search/?api=1&query=${params}&query_place_id=${
    data.place_id
  }">Go There!</a>
        </div>
    </div>
